...
{#if dataList != null}
  ...
  {#each dataList as data (data.id)}
    <tr
      ...
      class={classMap({
        ...
        "mdc-data-table__row--selected":
          !header && checkbox && checkbox.checked,
        ...internalClasses,
      })}
    ...
    >
      ...
      <Cell numeric>{data.maleMember}</Cell>
      <Cell numeric>{data.femaleMember}</Cell>
      <Cell>{data.workType}</Cell>
      <Cell>{data.work}</Cell>
      <Cell>{data.district}</Cell>
      <Cell>{data.village}</Cell>
      <Cell numeric>{data.rt}</Cell>
      <Cell numeric>{data.rw}</Cell>
      <Cell>{data.dateCreated.toDate()}</Cell>
    </tr>
  {/each}
{/if}
